...
get_state() {
	local state
	state=$(_pactl "get-$DEV_STATE" | awk '{print $2}')
...
get_volume() {
	_pactl "get-$DEV_VOLUME" | awk '{print $5}' | tr -d '%'
}
...
get_icon() {
	...
	if [[ $state == Muted ]]; then
		icon="$DEV_ICON-muted"
	elif ((new_level < MAX * 33 / 100)); then
		icon="$DEV_ICON-low"
...
set_volume() {
	...
	_pactl "set-$DEV_VOLUME" "$new_level%"
